...
public class FirstPersonAIO : MonoBehaviour
{
    ...
    public void HandleMouseRotate()
    {
        ...
        if (enableCameraMovement && !controllerPauseState)
        {
            ...
            {
                rotateInput.y = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.X ? Input.GetAxis("Mouse Y") : -Input.GetAxis("Mouse Y");
                rotateInput.x = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? Input.GetAxis("Mouse X") : -Input.GetAxis("Mouse X");
            }
            else
            {
                rotateInput.x = Input.GetAxis("Horizontal") * (mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? 1 : -1);
            }
    ...
    public void Rotate(Vector2 rotateInput)
    {
        ...
        targetAngles.y += rotateInput.x * (mouseSensitivity - ((baseCamFOV - camFOV) * fOVToMouseSensitivity) / 6f);
        if (cameraInputMethod == CameraInputMethod.Traditional) { targetAngles.x += rotateInput.y * (mouseSensitivity - ((baseCamFOV - camFOV) * fOVToMouseSensitivity) / 6f); }
    ...
    public void CalculateMoveDirection(Vector2 inputXY)
    {
    ...
    public void HandleMove(Vector2 inputXY)
    {
        CalculateMoveDirection(inputXY);
